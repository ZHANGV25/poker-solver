...
| 1.3 | Per-action EVs via σ̄-sampled walk | `src/mccfr_blueprint.c`, `precompute/export_v2.py`, `python/blueprint_v2.py`, `docs/PHASE_1_3_DESIGN.md` | ✅ **DONE 2026-04-11** | Phase A (local validation) complete 2026-04-09. Phase B EC2 run on 2026-04-09 silently produced a broken file (169/1.2B info sets visited) due to hash mixer incompatibility bug. Diagnosed and fixed across runs on 2026-04-11 — root cause: v2 training launched 2026-04-07 22:18 UTC which is 3.5h BEFORE commit 48da71b (splitmix64 replacement). Fix commits: e650a89 (legacy hash mixer flag) + 3062195 (split action-hash mixer from slot-derivation mixer) + 38231ab (atomic-counter progress prints). Valid `.bps` at 21M info sets visited (5M walk iters) uploaded 2026-04-11 19:13 UTC to `s3://poker-blueprint-unified-v2/unified_blueprint_v3_1.5B.bps`. The 5M walk coverage is sparse (p50=1 visit/IS); for production-quality EV estimates a 50M walk is recommended on the eventual 8B checkpoint. |
...
| Export tool (`precompute/export_v2.py`) | v3 + legacy-mixer dispatch (commits e650a89 + 3062195 + b2f9765) | writes `schema_version: 3` with BPR3 action-EVs section; hash_mixer tag = "boost" or "splitmix64" per run | Phase 1.3 implemented end-to-end; post-export sanity check blocks upload on failure. |
